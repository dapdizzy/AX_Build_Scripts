...
function Get-DefaultParameters
{
    ...
    $script:ClientRegistryPath       = (Set-Parameter "ClientRegistryPath" "HKCU:\SOFTWARE\Microsoft\Dynamics\6.0\Configuration" )
    $script:labelsFolder             = (Set-Parameter "LabelsFolder" "label files" )
    $script:gacutilExe               = (Set-Parameter "GacUtilPath" "C:\Program Files (x86)\Microsoft SDKs\Windows\v8.1A\bin\NETFX 4.5.1 Tools\x64\gacutil.exe")
}
...
function Run-CompilePartial
{
    ...
    Write-host "Running compile partial with the following arguments: $arguments" -ForegroundColor Cyan

    Write-Host "Compile partial output:" -ForegroundColor Yellow
...
function Compile-AX
{
    ...
    # Need to try to synchronize Views prior to synchronization, though it sounds weird, it may help prevent synchronization issues due to the presence of Views depending in other Views
...
function CreateSpecificXPOs([string]$xpoFileName)
{
    ...
    $systemClasses = @()
    foreach ($line in (Get-Content (Join-Path (Split-Path -Parent $script:startupDir) 'SystemClasses.txt')))
    {
        $systemClasses += $line.Trim()
    }
    $sysExclClasses = @()
    $sb = New-Object System.Text.StringBuilder
    foreach ($line in (Get-Content (Join-Path (Split-Path -Parent $script:startupDir) 'SysClassesExclusionList.txt')))
    {
        if ($sysExclClasses.Count -gt 0)
        {
            $sb.Append("|")
        }
        $sb.Append($line.Trim())
        $sysExclClasses += $line.Trim()
    }
    $exclClassRegEx = $sb.ToString()
    ...
    foreach ($line in $fileContents)
    {
        ...
        if ($shouldDecide -eq $true -and $line -match '; Microsoft Dynamics AX')
        {
            ...
            $isSystemClass = $false
            foreach ($class in $systemClasses)
            {
                if (($line -match "Class: $class") -and (-not ($line -match $exclClassRegEx)))
                {
                    $isSystemClass = $true
                    break
                }
            }
            # Views go into a separate file and are imported last 
            if ($line -match "View : (?<viewName>\w+)\b")
            {
                $viewsList += $Matches["viewName"]
                if (!$viewsWriter)
                {
                    $viewsXpoFileName = [System.IO.Path]::Combine([System.IO.Path]::GetDirectoryName($xpoFileName), ('{0}_views.xpo' -f [System.IO.Path]::GetFileNameWithoutExtension($xpoFileName)))
                    $viewsWriter = InitXpoWriter $viewsXpoFileName
                    if (!$writers)
                    {
                        $writers = new-object 'System.Collections.Generic.List[System.IO.StreamWriter]'
                    }
                    $writers.Add($viewsWriter)
                }
                $writer = $viewsWriter
            }
            ...
            elseif ($isSystemClass `
            # We don't need those system classes for now, they are already present in our baseline modelstore
            #-or ($line -match 'Class: SysImportElements' -or $line -match 'SysStartupCmd' -or $line -match 'Class: SysStartupCmdAOTImport' -or $line -match 'Class: SysStartupCmdCompilePartial' -or $line -match 'MMS_VerifyImportedAOTObjects') `
            )  # special case for SysImportElements
            {
                if (!$sysClsWriter)
                {
        ...
        # Consider only tables in this section (at least for LIPS), as we want Views to be explicitly present in it's own file
        # -or $line -match 'Element: VIE')
        {
            if (!$ddWriter)
            {
    ...
    # Build views PRCO for compiling the views right after importing their object definitions.
    $sb = new-object System.Text.STringBuilder
    $sb.AppendLine("<xml>")
    foreach ($view in $viewsList)
    {
        $sb.AppendLine("<type>View</type><name>$view</name><path>\\Data Dictionary\Views\$view</path>")
    }
    $sb.AppendLine("</xml>")
    $sb.ToString() | Out-File -FilePath (Join-Path (Split-Path -Path $xpoFileName -Parent) "Views_AOTprco.log") -Encoding default
}
...
function Import-AxCode([System.IO.FileSystemInfo]$model)
{
    ...
    # Remove AOTprco-like files before running AOT import and calling compile partial APIs
    Remove-AOTprocoLikeFiles $script:clientLogDir
    ...
    if ($sysClsXpoFileName -ne $null -and $sysClsXpoFileName  -ne '')
    {
        ...
        # Use AX Startup CMD instead of built-in zotimport flag, as we want to eventually compile the imported XPO )at least give it a good try).

        Run-AOTImport (Join-Path $currentLogFolder $sysClsXpoFileName)
        ...
        # Run compiler partial only if we have really have something to compile
        Compile-AOTprcoDepsrecursively 5 # Hardcode five attempt for now (will move to the parameters file in the future)
        ...
        Compile-AOTprcoDepsrecursively 5 # five attempt is hardcoded fot the period of testing
    }
    ...
    if ($sysTabXpoFileName -ne $null -and $sysTabXpoFileName -ne '')
    {
        ...
        Run-AOTImport (join-path $currentLogFolder $sysTabXpoFileName)
        ...
        Compile-AOTprcoDepsrecursively 5
    }
    ...
    foreach ($xpoName in (gci -Path "$currentLogFolder\*" -Include "$xpoNamePrefix*.xpo" -Exclude '*_sys*.xpo' -Name)) # Handle sysTab (System Tables) & sysCls (System Classes) separately
    {
        ...
        -or ($xpoName -eq 'Combined.VAR Model_sec.xpo')) # Import Security stuff twice as well, as the system seems to skip some (at least one) objects while importing this file.
        {
    ...
    Write-Host "Importing Views the last turn $(Get-Date)" -ForegroundColor Cyan

    Import-XPO "Combined.VAR Model_views.xpo"
    # I love views so much, that I can't stand, but to import them twice, once I have a chance to do it!
    Import-XPO "Combined.VAR Model_views.xpo"

    Write-Host "Views have been imported $(Get-Date)" -ForegroundColor Cyan

    Write-Host "Security goes the very last turn, once again" -ForegroundColor Green

    Import-XPO 'Combined.VAR Model_sec.xpo'

    # Import this nasty privilage that does not want to be imported after 2 times importing combined VAR Layer xpo and 3 times importing Security objects in a dedicated XPO file
    $local:privilageFileName = Join-Path $ApplicationSourceDir "$script:ModelName\Security\Privilages\MMS_LogisticWorkStatusChange_Close.xpo"
    if ((Test-Path $local:privilageFileName) -eq $true)
    {
        Import-XPO $local:privilageFileName $true
    }
    
    Write-Host "Running Compile partial with the views list at $(Get-Date)" -ForegroundColor Green

    Copy-Item -Path (Join-Path $currentLogFolder "Views_AOTprco.log") -Destination (Join-Path "$script:clientLogDir" "AOTprco.log") -Force

    Write-Host "Updated AOTprco.log in $script:clientLogDir at $(Get-Date)" -ForegroundColor Cyan

    Remove-AOTprocoLikeFiles $script:clientLogDir

    # Time to compile the views after we've imported them in the last turn.
    Run-CompilePartial

    Compile-AOTprcoDepsrecursively 5

    # At least we've tried to compile the views  by now and hopefully we're not going to have any issues in regards to them at XA Synchronization phase that follows right after.
...
function Compile-AOTprcoDepsrecursively([int]$numberOfAttempts = 5)
{
    $retryCount = $numberOfAttempts
    if ($retryCount -le 0)
    {
        $retryCount = 5 # Hardcode five attempts for now (move to parameters file in the future)
    }
    $hasSomethingToBuild = Merge-AOTprcoLikeFiles $script:clientLogDir
    # Run compiler partial only if we have really have something to compile
    if ($hasSomethingToBuild -eq $true)
    {
        do
        {
            Remove-AOTprocoLikeFiles $script:clientLogDir
            ...
            $hasSomethingToBuild = Merge-AOTprcoLikeFiles $script:clientLogDir
            $retryCount--
        } while ($hasSomethingToBuild -and $retryCount -gt 0)
    }
}

function Remove-AOTprocoLikeFiles([string]$logDir)
{
    Remove-Item -Path (Join-Path $logDir "*_AOTprco.log") -ErrorAction SilentlyContinue    
}

function Merge-AOTprcoLikeFiles([string]$logFolder)
{
    $sb = New-Object System.Text.StringBuilder
    gci -Path "$logFolder\*" -Include "*_AOTprco.log" -ErrorAction SilentlyContinue `
    |% {$sb.AppendLine((Get-Content $_.FullName))}
    if ($sb.Length -gt 0)
    {
        "<xml>`n$($sb.ToString())</xml>" | Out-File -FilePath "$logFolder\AOTprco.log" -Encoding default -ErrorAction SilentlyContinue
        return $true
    }
    return $false
}

function Run-AOTImport([string]$fileName)
{
    if (!$ImportTimeout)
    {
        $ImportTimeout = 10
    }
    $arguments = ('-lazyclassloading -lazytableloading "-StartupCmd=aotimport_{0}" -internal=noModalBoxes' -f $fileName)
    $axProcess = Start-Process $ax32 -WorkingDirectory $clientBinDir -PassThru -WindowStyle minimized -ArgumentList $arguments -OutVariable out
    Write-InfoLog $out
    if ($axProcess.WaitForExit(60000*$ImportTimeout) -eq $false)
    {
...
{
    $layerParm = ''
    if ($axLayer -ne '') {$layerParm = '-aol={0}' -f $axLayer}
    $aolParm = ''
    if ($aolCode -ne '') {$aolParm = '-aolCode={0}' -f $aolCode}
    $arguments = '{0} {1} -lazyclassloading -lazytableloading -StartupCmd=compilepartial -novsprojcompileall -internal=noModalBoxes' -f $layerParm, $aolParm
    Write-host ("Calling CompilePartial API : {0}" -f (Get-Date)) 
    $axProcess = Start-Process $ax32 -WorkingDirectory $clientBinDir -PassThru -WindowStyle minimized -ArgumentList $arguments -OutVariable out
    Write-host $out
    Write-InfoLog (" ")
...
function Install-PackagesToGAC($packagesFolder)
{
    ...
    Write-Host "GacUtil is '$gacutilExe'  ***  " -ForegroundColor Cyan
    ...
    Write-Host "Local packages folder: $localPackagesFolder" -ForegroundColor Cyan

    $dlls = gci -Path "$localPackagesFolder" -File -Include *.dll -Recurse # | Select-Object -Property FullName
    if ($dlls -eq $null)
    {
        Write-Host "No dlls have been found" -ForegroundColor Cyan
    }
    ForEach ($dll in $dlls)
    {
        Write-Host $dll -ForegroundColor Cyan
        $p = Start-Process $gacutilExe -ArgumentList ('/i "{0}"' -f $dll) -OutVariable out -PassThru
...
function Clean-Build
{
    ...
    # Do not clear models as we rely on the valid baseline db backup instead
    <#
...
function Restore-Database($dbName, $backupFileName)
{
    ...
    $relocateData = @()
    $svr = new-object ('Microsoft.SqlServer.Management.Smo.Server') -ArgumentList $sqlServer
...
{
    $exclusionList = @()
    # It is always valid to drop all the TFS source controlled files on the Build machine, so no exclusion list whatsoever
    if ($script:scriptName -ne 'BUILD')
    {
...
function Remove-OldSourceControlledFiles([string]$sourceDir)
{
    ...
    # Powershell seems to have different interpretation of "path" vs "path\*" in regards to commands line gci or Remove-Item
    # Had to add \* to the path, as nothing seems to be deleted on LIPS test build box, while it seemed to wirk as expected on my local dev box
